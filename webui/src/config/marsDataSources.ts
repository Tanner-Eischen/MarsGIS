...
export type TemporalType = 'static' | 'temporal'

export interface MarsOverlayDefinition {
  name: OverlayType
  displayName: string
  icon: string
  description: string
 ...
 */
export const MARS_OVERLAY_DEFINITIONS: MarsOverlayDefinition[] = [
  {
    name: 'elevation',
 ...
 */
export function getOverlayDefinition(name: OverlayType): MarsOverlayDefinition | undefined {
 ...
 */
export function getOverlaysForDataset(dataset: MarsDataset): MarsOverlayDefinition[] {
 ...
 */
export function isTemporalOverlay(overlayType: OverlayType): boolean {
 ...
 */
export function getTemporalParams(overlayType: OverlayType): string[] {
  const def = getOverlayDefinition(overlayType)
  if (!def?.temporalParams) return []
  
  const params: string[] = []
  if (def.temporalParams.marsSol) params.push('marsSol')
  if (def.temporalParams.season) params.push('season')
  if (def.temporalParams.timeOfDay) params.push('timeOfDay')
  if (def.temporalParams.dustStormPeriod) params.push('dustStormPeriod')
  if (def.temporalParams.sunAzimuth) params.push('sunAzimuth')
  if (def.temporalParams.sunAltitude) params.push('sunAltitude')

...
class NavigationEngine:
    ...
    def _normalize_resolution_m(self, resolution_value: float, latitude_deg: float) -> float:
        """Normalize DEM resolution to meters per pixel."""
        if resolution_value >= 1.0:
            return resolution_value

        # Geographic rasters store degree-per-pixel; convert to meters on Mars.
        meters_per_degree = (2.0 * math.pi * self.config.mars.equatorial_radius_m) / 360.0
        lon_scale = max(math.cos(math.radians(latitude_deg)), 0.1)
        converted = max(resolution_value * meters_per_degree * lon_scale, 1.0)
        logger.debug(
            "Converted DEM resolution from degrees to meters",
            resolution_deg=resolution_value,
            latitude_deg=latitude_deg,
            resolution_m=converted,
        )
        return converted
    ...
    def plan_to_site(
        ...
        try:
            ...
            if "mola" in self.config.data_sources:
                ...
                cell_size_m = self._normalize_resolution_m(
                    float(dem.attrs.get('resolution_m', 463.0)),
                    start_lat,
                )
            ...
            # Run A* pathfinding
            resolution_raw = float(dem.attrs.get('resolution_m', cell_size_m))
            resolution_m = self._normalize_resolution_m(resolution_raw, start_lat)
            ...
            max_spacing_pixels = max(1, int(max_waypoint_spacing_m / (resolution_m * downsample_factor)))
            ...
            for i, (lat, lon, elev) in enumerate(waypoints_latlon):
                ...
                waypoints_site.append({
                    ...
                    'x_meters': x,
                    'y_meters': y,
                    # Backward-compatible aliases expected by existing tests/clients.
                    'x_site': x,
                    'y_site': y,
                    'z_site': z,
                    'lat': lat,
